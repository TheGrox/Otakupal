...
def chat():
    ...
    # Re-check if current_chat_id is valid for the user, as a safeguard
    user_chat_sessions = get_chat_sessions_for_user(user_id)
    valid_chat_ids = [s['id'] for s in user_chat_sessions]
    if not current_chat_id or current_chat_id not in valid_chat_ids:
        # If current_chat_id is missing or invalid, create a new one for the user
        current_chat_id = create_new_chat_session_for_user(user_id)
        session['current_chat_id'] = current_chat_id
        session['chat_messages'] = []

    messages_for_llm = session.get('chat_messages', []) + [{"role": "user", "content": user_message}]
    ...
    session['chat_messages'].append({"role": "bot", "content": response_content})

    if len(session['chat_messages']) == 2:
        ...
        return jsonify({'response': response_content, 'current_chat_id': current_chat_id, 'refresh_history': True})

    return jsonify({'response': response_content, 'current_chat_id': current_chat_id, 'refresh_history': False})
