...
function regenerateResponse(button) {
    const messageDiv = button.closest('.message');
    const contentDiv = messageDiv.querySelector('.content');
    const originalMessage = contentDiv.innerText;

    // Call your chat function to regenerate the response
    // For example, you can send the original message to the server and get a new response
    // Example: sendMessage(originalMessage);
}